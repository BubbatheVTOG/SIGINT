...
> ## MIGRATION TODO — delete this section once complete on both hosts
>
> A theme-externalization migration is in progress. **matt is done; sigint
> is not.** On sigint, run the steps below, then delete this section.
>
> Goal: move all color values out of committed configs into a gitignored
> `~/.config/theme/` directory of generated palette files (fixes: live theme
> state polluting `git status` and causing cross-host merge conflicts). After
> migration, toggling themes changes zero committed files. **matt already has
> the new committed configs** (pull to get them); sigint just needs the local
> migration steps below.
>
> **Steps (run on sigint only — matt is already migrated):**
>
> 1. `cd ~/git/sigint && git pull --ff-only` (gets the new committed configs
>    from matt: kitty.conf, tmux.conf, waybar style.css, rofi theme.rasi,
>    sigint hyprland.lua/hyprlock.conf/hyprpaper.conf, toggle-theme.sh, etc.)
> 2. `cd dotfiles && stow -R -t ~ common sigint` (refresh symlinks)
> 3. Remove the old stow-managed theme dir and create a real one:
>    ```bash
>    rm ~/.config/theme          # remove stow symlink to repo
>    mkdir -p ~/.config/theme    # real, gitignored, not stowed
>    ```
> 4. Bootstrap palette files: `~/.local/bin/toggle-theme.sh --init`
>    (generates 8 files into `~/.config/theme/`)
> 5. Verify:
>    ```bash
>    luajit -e 'assert(loadfile("dotfiles/sigint/.config/hypr/hyprland.lua"))'
>    tmux source-file ~/.tmux.conf
>    zsh -c true
>    git status        # should be clean (only this AGENTS.md note if uncommitted)
>    ls ~/.config/theme/  # 8 palette files
>    ```
> 6. Restart waybar/hyprpaper/kitty if they're running stale, or just
>    `hyprctl reload` + `pkill -SIGUSR2 waybar`.
> 7. **Delete this section** once sigint is migrated and `git status` is
>    clean after a toggle on both hosts.
>
> Background: committed configs now contain only `include`/`source`/`@import`/
> `dofile` lines pointing at `~/.config/theme/` (a gitignored real directory
> generated by `toggle-theme.sh`). `dofile` (not `require`) in hyprland.lua
> re-reads on every `hyprctl reload` so theme switches take effect
> immediately; `require` caches in `package.loaded`. The `pcall` guard
> handles the missing-file case on first boot (falls back to inline teal
> defaults, then `--init` autostart guard populates the real files).
